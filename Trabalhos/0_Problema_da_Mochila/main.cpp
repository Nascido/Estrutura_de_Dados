...
void insertion_sort(std::vector<std::tuple<int, int, int, int, double>>& vetor){
...
int main(){
    ...
    insertion_sort(itensDisponiveis);                // Ordenar itens pela relação u/w
    ...
    while (temEspaco && !finalizado) {
      for (std::size_t i=0; i<tamanho ; i++) {
        ...
        int iItem = std::get<0>(item);
        int wItem = std::get<2>(item);
        int dItem = std::get<3>(item);
